...
def test_sum_2d_mask(siffreaders):
    ...
    NUM_ROIS = 7
    rois = np.random.rand(NUM_ROIS, *corrosiff_sr.frame_shape()) > 0.3

    assert (
        corrosiff_sr.sum_rois(rois, registration=None)
        == siffc_sr.sum_rois(rois, registration=None)
    ).all()

    assert not (
        corrosiff_sr.sum_rois(rois, frames = framelist, registration=None)
        == siffc_sr.sum_rois(rois, frames = framelist, registration=dummy_reg)
    ).all()

    assert (
        corrosiff_sr.sum_rois(rois, frames = framelist, registration=dummy_reg)
        == siffc_sr.sum_rois(rois, frames = framelist, registration=dummy_reg)
    ).all()
...
#         # Whole volume agrees
        ...
        assert (
            siffc_sr.sum_rois(complicated_rois[k-1], frames = list(range(N_FRAMES)), registration=None
            ) == corrosiff_sr.sum_rois(complicated_rois[k-1], frames = list(range(N_FRAMES)), registration=None
            )
        ).all()
